...
<div class="max-w-3xl mx-auto animate-fade-in">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold text-slate-800">تعديل العنوان: {{ $address->title }}</h2>
    </div>

    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.addresses.update', $address) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <!-- Title -->
                <div>
                    <label for="title" class="block text-sm font-bold text-slate-700 mb-2">اسم العنوان <span class="text-rose-500">*</span></label>
                    <input type="text" 
                ...
                <div>
                    ...
                    <textarea class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-start focus:ring-4 focus:ring-primary-start/10 transition-all-300 outline-none @error('address') border-rose-500 ring-rose-500/10 @enderror" 
                              id="address" name="address" rows="4">{{ old('address', $address->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" id="status" name="status" value="1" class="sr-only peer" {{ old('status', $address->status) ? 'checked' : '' }}>
