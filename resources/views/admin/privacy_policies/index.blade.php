...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 animate-fade-in">
    <div>
        <h2 class="text-3xl font-bold text-slate-800">سياسة الخصوصية</h2>
        <p class="text-slate-500 mt-1">إدارة محتوى صفحة سياسة الخصوصية.</p>
    </div>
    <a href="{{ route('admin.privacy_policies.create') }}" class="inline-flex items-center justify-center px-6 py-3 bg-primary-start hover:bg-primary-end text-white font-bold rounded-2xl shadow-lg hover:shadow-primary-start/30 transition-all-300">
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($privacyPolicies as $policy)
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        <td class="px-6 py-4 text-slate-400 text-sm">{{ $policy->id }}</td>
                        <td class="px-6 py-4">
                            <div class="font-bold text-slate-700 group-hover:text-primary-start transition-colors max-w-md">
                                {{ $policy->title }}
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.privacy_policies.show', $policy) }}" class="p-2 text-primary-start hover:bg-primary-start/10 rounded-xl transition-all-300" title="عرض">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.privacy_policies.edit', $policy) }}" class="p-2 text-amber-500 hover:bg-amber-50 rounded-xl transition-all-300" title="تعديل">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.privacy_policies.destroy', $policy) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-rose-500 hover:bg-rose-50 rounded-xl transition-all-300" 
                                            data-confirm data-confirm-title="حذف سياسة الخصوصية" 
                                            data-confirm-message="هل أنت متأكد من رغبتك في حذف هذا القسم؟" 
                                            title="حذف">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-slate-400">
                            ...
                            <p>لا توجد أقسام لسياسة الخصوصية حالياً</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
