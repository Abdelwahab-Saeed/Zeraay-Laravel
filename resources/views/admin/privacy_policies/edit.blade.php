...
<div class="max-w-3xl mx-auto animate-fade-in">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold text-slate-800">تعديل سياسة الخصوصية</h2>
    </div>

    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.privacy_policies.update', $privacyPolicy) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="title" name="title" value="{{ old('title', $privacyPolicy->title) }}" placeholder="أدخل العنوان">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-start focus:ring-4 focus:ring-primary-start/10 transition-all-300 outline-none @error('content') border-rose-500 ring-rose-500/10 @enderror" 
                              id="content" name="content" rows="10" placeholder="اكتب المحتوى هنا...">{{ old('content', $privacyPolicy->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" id="status" name="status" value="1" class="sr-only peer" {{ old('status', $privacyPolicy->status) ? 'checked' : '' }}>
                ...
                <div class="flex items-center gap-4 pt-4 border-t border-slate-100">
                    <button type="submit" class="px-8 py-3 bg-primary-start hover:bg-primary-end text-white font-bold rounded-2xl shadow-lg hover:shadow-primary-start/30 transition-all-300 flex-1 flex justify-center items-center">
                        <i class="fas fa-save ml-2"></i> تحديث القسم
                    </button>
