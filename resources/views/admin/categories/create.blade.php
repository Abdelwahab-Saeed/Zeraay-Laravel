...
<div class="max-w-3xl mx-auto animate-fade-in">
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-bold text-slate-800">إضافة فئة جديدة</h2>
    </div>

    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.categories.store') }}" method="POST" novalidate enctype="multipart/form-data" class="p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="name" name="name" value="{{ old('name') }}" placeholder="أدخل اسم الفئة">
                    @error('name')
                ...
                <div>
                    ...
                    <textarea class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-start focus:ring-4 focus:ring-primary-start/10 transition-all-300 outline-none @error('description') border-rose-500 ring-rose-500/10 @enderror" 
                              id="description" name="description" rows="4" placeholder="اكتب وصفاً قصيراً للفئة...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Image -->
                <div>
                    <label for="image" class="block text-sm font-bold text-slate-700 mb-2">صورة الفئة</label>
